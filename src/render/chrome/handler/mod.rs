...
use crate::render::chrome_protocol::cdp::events::CdpEventMessage;
use crate::render::chrome_wire::{CallId, Message, Method, Response};
use crate::render::chrome_wire::{MethodId, Request as CdpRequest};
pub(crate) use page::PageInner;

use crate::render::chrome::cmd::{to_command_response, CommandMessage};
use crate::render::chrome::conn::Connection;
use crate::render::chrome::error::{CdpError, Result};
use crate::render::chrome::handler::browser::BrowserContext;
use crate::render::chrome::handler::frame::FrameNavigationRequest;
use crate::render::chrome::handler::frame::{NavigationError, NavigationId, NavigationOk};
use crate::render::chrome::handler::job::PeriodicJob;
use crate::render::chrome::handler::session::Session;
use crate::render::chrome::handler::target::TargetEvent;
use crate::render::chrome::handler::target::{Target, TargetConfig};
use crate::render::chrome::handler::viewport::Viewport;
use crate::render::chrome::page::Page;

/// Standard timeout in MS (default 30s).
///
/// Used as the default when no per-call override is plumbed through. Heavy
/// real-world targets (Cloudflare-fronted SPAs, ad-laden landing pages) can
/// exceed 30s for `Page.navigate` to settle on the auto-fetched
/// Chromium-for-Testing build. Operators bump via `CRAWLEX_REQUEST_TIMEOUT_MS`
/// without rebuilding — the constant is read once at process start through
/// [`request_timeout_ms`] which falls back to this value.
...
pub fn request_timeout_ms() -> u64 {
    ...
    *CACHE.get_or_init(|| {
        std::env::var("CRAWLEX_REQUEST_TIMEOUT_MS")
            .ok()
    ...
    fn on_target_created(&mut self, event: EventTargetCreated) {
        ...
        let target = Target::new(
            ...
            TargetConfig {
                ...
                stealth: self.config.stealth.clone(),
            },
            browser_ctx,
        );
...
pub struct HandlerConfig {
    ...
    /// Stealth knobs forwarded from `BrowserConfig`. Grouping here lets
    /// the handler pass them wholesale to `TargetConfig` without growing
    /// a new field each time a stealth option is added.
