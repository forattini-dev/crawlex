...
use crate::render::chrome_protocol::cdp::browser_protocol::browser::{
    GetVersionParams, GetVersionReturns,
};
...
use crate::render::chrome_protocol::cdp::browser_protocol::target::{
    ActivateTargetParams, SessionId, TargetId,
};
...
use crate::render::chrome_wire::{Command, CommandResponse};

use crate::render::chrome::cmd::{to_command_response, CommandMessage};
...
use crate::render::chrome::handler::target::{GetExecutionContext, TargetMessage};
use crate::render::chrome::handler::target_message_future::TargetMessageFuture;
use crate::render::chrome::js::EvaluationResult;
use crate::render::chrome::layout::Point;
use crate::render::chrome::page::ScreenshotParams;
use crate::render::chrome::{keys, utils, ArcHttpRequest};
...
impl PageInner {
    ...
    pub async fn click(&self, point: Point) -> Result<&Self> {
        let default_opts = crate::render::chrome_wire::ClickOptions::default();
    ...
    pub async fn click_with(
        &self,
        point: Point,
        options: crate::render::chrome_wire::ClickOptions,
    ) -> Result<&Self> {
    ...
    pub async fn evaluate_expression(
        ...
        if evaluate.context_id.is_none() {
            evaluate.context_id = self.preferred_execution_context().await?;
        }
    ...
    pub async fn evaluate_function(
        ...
        if evaluate.execution_context_id.is_none() {
            evaluate.execution_context_id = self.preferred_execution_context().await?;
        }
        if evaluate.await_promise.is_none() {
            evaluate.await_promise = Some(true);
        }
    ...
    /// Pick the best available execution context for an evaluate call.
    /// Prefers the main world; falls back to the secondary (isolated)
    /// world when the main world has no bound context — this is the
    /// steady state under stealth mode
    /// (`BrowserConfig::stealth_runtime_enable_skip = true`), where
    /// `Runtime.enable` is suppressed and therefore
    /// `Runtime.executionContextCreated` never fires for the default
    /// context. The isolated world still has a valid context because
    /// `Page.createIsolatedWorld`'s response binds it directly (see
    /// `FrameManager::on_create_isolated_world_response`).
    ///
    /// **Semantic implication for callers** — under stealth the
    /// evaluation runs in the isolated world: DOM is shared with main,
    /// but JS globals (`window.foo`, module-level variables) are not.
    /// Stash state on DOM nodes or on the Rust side; see
    /// `BrowserConfigBuilder::stealth_runtime_enable_skip` docs for the
    /// full contract.
    async fn preferred_execution_context(&self) -> Result<Option<ExecutionContextId>> {
        if let Some(ctx) = self.execution_context().await? {
            return Ok(Some(ctx));
        }
        let secondary = self.secondary_execution_context().await?;
        if secondary.is_none() {
            tracing::warn!(
                target: "crawlex::stealth",
                "no execution context available for evaluate; if stealth_runtime_enable_skip=true, \
                 Page.createIsolatedWorld response→context binding is the fase 4 TODO"
            );
        }
        Ok(secondary)
    }
