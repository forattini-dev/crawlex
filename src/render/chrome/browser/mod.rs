...
use crate::render::chrome_protocol::cdp::browser_protocol::network::{Cookie, CookieParam};
use crate::render::chrome_protocol::cdp::browser_protocol::storage::{
...
use crate::render::chrome_protocol::cdp::{CdpEventMessage, IntoEventKind};
use crate::render::chrome_wire::*;

pub use self::config::{
    BrowserConfig, BrowserConfigBuilder, HeadlessMode, StealthConfig, LAUNCH_TIMEOUT,
};
use crate::render::chrome::async_process::{Child, ExitStatus};
use crate::render::chrome::cmd::{to_command_response, CommandMessage};
use crate::render::chrome::conn::Connection;
use crate::render::chrome::error::{BrowserStderr, CdpError, Result};
use crate::render::chrome::handler::browser::BrowserContext;
use crate::render::chrome::handler::{Handler, HandlerConfig, HandlerMessage};
use crate::render::chrome::listeners::{EventListenerRequest, EventStream};
use crate::render::chrome::page::Page;
use crate::render::chrome::utils;

mod argument;
mod config;

/// A [`Browser`] is created when the CDP client connects to a Chromium instance.
#[derive(Debug)]
pub struct Browser {
    ...
    pub async fn set_cookies(&self, mut cookies: Vec<CookieParam>) -> Result<&Self> {
        for cookie in &mut cookies {
            if let Some(url) = cookie.url.as_ref() {
                crate::render::chrome::page::validate_cookie_url(url)?;
            }
        }
