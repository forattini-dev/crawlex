...
use crate::render::chrome_protocol::cdp::browser_protocol::target::SessionId;
use crate::render::chrome_wire::{Command, CommandResponse, Method, MethodId, Request, Response};

use crate::render::chrome::error::{CdpError, DeadlineExceeded, Result};
use crate::render::chrome::handler::request_timeout_ms;
...
impl CommandChain {
    ...
    /// Method identifiers queued in this chain, in the order they will be
    /// sent. Useful for diagnostics (logging what init sequence we're
    /// firing at a new target) and for stealth-audit tests that assert
    /// `Runtime.enable` is absent from a stealth init command set.
    pub fn method_identifiers(&self) -> Vec<String> {
        self.cmds
            .iter()
            .map(|(m, _)| m.as_ref().to_string())
            .collect()
    }
