...
// ─────────────────────────────────────────────────────────────────────
// `crawlex update-blocklist`
// ─────────────────────────────────────────────────────────────────────

/// Fetch an EasyList-style feed (or read it from a local file) and
/// write the extracted domain rules into the override file consulted
/// by `crate::adblock` at runtime. The override merges *on top of* the
/// embedded baseline — there is no destructive overwrite of bundled
/// entries.
async fn cmd_update_blocklist(args: args::UpdateBlocklistArgs) -> anyhow::Result<()> {
    use crate::adblock::{default_override_path, BlockList};
    ...
    // Dump to a deterministic-sorted text file so successive runs
    // produce stable diffs. Reuse `BlockList`'s normalisation rules.
    let mut seen: BTreeSet<String> = BTreeSet::new();
    for line in body.lines() {
        if let Some(host) = crate::adblock::extract_domain(line) {
            seen.insert(host);
        }
    }
    // Round-trip sanity check: list should be reconstructable from the
    // serialised form. Fail loudly if a future parser tweak silently
    // drops everything.
    let mut list = BlockList::empty();
    list.extend_from_str(&body);
    debug_assert_eq!(list.len(), seen.len());
    ...
    let mut out = String::with_capacity(seen.len() * 24);
    out.push_str("# crawlex blocklist override — generated by `crawlex update-blocklist`.\n");
    out.push_str("# Merged with the embedded baseline at runtime; do not edit by hand.\n");
    if let Some(src) = &args.from_file {
    ...
    std::fs::write(&out_path, out)
        .map_err(|e| anyhow::anyhow!("write {}: {}", out_path.display(), e))?;

    eprintln!(
        "update-blocklist: wrote {} domain(s) to {} (baseline list still applies)",
        seen.len(),
        out_path.display()
    );
    Ok(())
...
async fn fetch_blocklist_source(url: &str) -> anyhow::Result<String> {
    let resp = reqwest::get(url)
        .await
        .map_err(|e| anyhow::anyhow!("fetch {}: {}", url, e))?;
    if !resp.status().is_success() {
        return Err(anyhow::anyhow!("fetch {}: HTTP {}", url, resp.status()));
    }
    resp.text()
        .await
        .map_err(|e| anyhow::anyhow!("read body from {}: {}", url, e))
}

#[cfg(not(feature = "cdp-backend"))]
async fn fetch_blocklist_source(_url: &str) -> anyhow::Result<String> {
    Err(anyhow::anyhow!(
